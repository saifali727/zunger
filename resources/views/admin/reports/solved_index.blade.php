...
@push('start-script')
    ...
    <script>
        function delete_post(id,$report_id) {
            swal.fire({
                title: "Are you sure?",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: '{{ url('admin/post/delete') }}/' + id + '/' + $report_id,
        ...
        function banned_user(id,$report_id) {
            swal.fire({
                title: "Are you sure?",
                text: "User will not be able to use the app anymore",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                confirmButtonText: "Yes, bann the user!",
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: '{{ url('admin/user/banned') }}/' + id + '/' + $report_id,

                        success: function(data) {
                            $('#default-ordering').DataTable().ajax.reload();
                            swal.fire("Success", "User disabled successfully to reactivate it go to users section", "success");
                        },
                        error: function(error) {
                            Snackbar.show({
                                text: 'Server error',
